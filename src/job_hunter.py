    ...
    for query in config["searches"]:
        for location in config["locations"]:
            ...
            for start in range(0, cap, 10):
                params = urlencode({"keywords": query, "location": location, "start": start, "f_TPR": "r3888000"})
...
def main() -> int:
    ...
    if args.fixture:
        ...
        blocked_companies = config.get("blocked_companies", [])
        jobs = filter_blocked_companies(jobs, blocked_companies)
        previous = load_previous(ROOT / "state" / "jobs.json")
        for jid, old in previous.items():
            if jid not in {job.id for job in jobs} and filter_blocked_companies([old], blocked_companies):
                jobs.append(old)
